...
const ContentPage = ({id}:{id:any}) => {
  ...
  if (!data) return <div className='flex justify-center items-center'><Loading></Loading></div>
      ...
      <div className="max-w-sm mx-auto rounded-2xl overflow-hidden shadow-lg bg-white">
        ...
        <div className="relative -mt-10 mx-4 bg-white p-4 rounded-xl shadow-md z-10">
          ...
          <div className="flex items-center text-sm text-gray-500 mt-1">
            ...
            {data.total_tickets === 0 && (<div className="mt-2 text-sm text-red-600 font-medium">HẾT VÉ</div>)}
            {data.total_tickets !== 0 && (<div className="mt-2 text-sm text-green-600 font-medium">Còn lại: {data.total_tickets}</div>)} 
        </div>
  ...
  disabled={data.total_tickets === 0}
>
  {data.total_tickets === 0 ? 'Hết vé' : 'Mua vé ngay'}
</button>
